...
if [ -f "/tmp/project.tar.gz" ]; then
    echo "Successfully created project.tar.gz in /tmp"
    # Optional: You can add a size check here
    tar_size=$(du -h /tmp/project.tar.gz | cut -f1)
    echo "Size of project.tar.gz: $tar_size"
else
...
response=$(curl -X POST -H "X-API-KEY: $INPUT_API_TOKEN" \
           ...
           -d "{\"object_key\": \"project-$current_datetime.tar.gz\", \"bucket_name\": \"protoml-projects-prod\"}" \
...
curl -X PUT -T /tmp/project.tar.gz "$upload_url"
